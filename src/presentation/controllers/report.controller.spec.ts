...
describe('ReportController', () => {
  ...
  it('delegates getting revenue report to the revenue use case', async () => {
    ...
    await expect(controller.getRevenue('2026-06', 'center-1', 'class-1')).resolves.toEqual({ some: 'data' });
    expect(revenueReport.execute).toHaveBeenCalledWith({ month: '2026-06', centerId: 'center-1', classId: 'class-1' });
  });
  ...
  it('delegates getting salary report to the salary use case', async () => {
    ...
    await expect(controller.getSalary('2026-06', 'center-1')).resolves.toEqual({ salary: 'data' });
    expect(salaryReport.execute).toHaveBeenCalledWith({ month: '2026-06', centerId: 'center-1' });
  });
  ...
  it('delegates getting class students stats report to reportsQuery', async () => {
    ...
    expect(reportsQuery.getClassStudentsStats).toHaveBeenCalledWith({ centerId: 'center-1', classId: 'class-1' });
  });
  ...
  it('delegates getting sale orders report to reportsQuery', async () => {
    ...
    await expect(controller.getSaleOrders('2026-06', 'center-1', 'class-1')).resolves.toEqual([{ billId: '1', status: 'Paid' }]);
    expect(reportsQuery.getSaleOrdersReport).toHaveBeenCalledWith({ month: '2026-06', centerId: 'center-1', classId: 'class-1' });
  });
  ...
  it('delegates getting class attendance report to reportsQuery', async () => {
    ...
    await expect(controller.getClassAttendance('2026-06', 'center-1', 'class-1')).resolves.toEqual([{ classId: '1', rate: 90 }]);
    expect(reportsQuery.getAttendanceByClass).toHaveBeenCalledWith({ month: '2026-06', centerId: 'center-1', classId: 'class-1' });
  });
  ...
  it('delegates getting student attendance report to reportsQuery', async () => {
    ...
    await expect(controller.getStudentAttendance('2026-06', 'center-1', 'class-1')).resolves.toEqual([{ studentId: '1', rate: '95' }]);
    expect(reportsQuery.getStudentAttendanceReport).toHaveBeenCalledWith({ month: '2026-06', centerId: 'center-1', classId: 'class-1' });
  });
  ...
  it('delegates getting student debts report to reportsQuery', async () => {
    ...
    await expect(controller.getStudentDebts('2026-06', 'center-1', 'class-1')).resolves.toEqual([{ studentId: '1', debtAmount: 100 }]);
    expect(reportsQuery.getStudentDebtsReport).toHaveBeenCalledWith({ month: '2026-06', centerId: 'center-1', classId: 'class-1' });
  });
});
