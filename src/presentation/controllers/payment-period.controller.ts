      ...
      for (const [studentId, attList] of studentMap.entries()) {
        ...
        for (const [classId, group] of classMap.entries()) {
          ...
          for (const att of group.atts) {
            ...
            // Find pricing effective for the session date
          ...
          if (group.atts.length > 0) {
            billItemsData.push({
              classId,
              className: group.classEntity.className || group.classEntity.name || '',
              courseName: group.classEntity.course?.name || '',
              levelName: group.classEntity.courseLevel?.levelName || '',
              sessionsCount: group.atts.length,
              rate: latestRate,
              totalAmount: classTotalAmount,
            });
            studentTotalAmount += classTotalAmount;
          }
        }
      ...
      for (const [teacherId, sessionList] of teacherMap.entries()) {
        ...
        for (const [classId, group] of classMap.entries()) {
          ...
          if (group.sessions.length > 0) {
            ...
            teacherTotalAmount += classTotalAmount;
          }
        }
