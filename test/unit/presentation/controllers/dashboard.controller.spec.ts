...
describe('DashboardController views', () => {
  ...
  describe('student dashboard view', () => {
    it('correctly calculates attendance, homework statistics, and comments', async () => {
      ...
      const classStudentRepo = {
        find: jest.fn().mockResolvedValue([{ classId: 'class-1', studentId: 'student-1', status: 'Active', joinedDate: '2020-01-01' }]),
      };
      ...
      const mockSessions = [
        ...
        },
        {
          id: 'session-3',
          classId: 'class-1',
          date: `${currentYear}-${String(Number(currentMonth) % 12 + 1).padStart(2, '0')}-03`,
          status: 'Scheduled',
          classEntity: { className: 'Toán' },
        }
      ];
      ...
      const submissionRepo = {
        find: jest.fn().mockResolvedValue([
          { id: 'sub-1', assignmentId: 'assign-1', status: 'graded', score: 9, feedback: 'Giỏi', gradedAt: new Date(`${currentYear}-${currentMonth}-01T10:00:00Z`) }
        ]),
      };
      ...
      expect(result.studentInfo.name).toBe('Nguyen An');
  ...
  describe('Teacher Assistant dashboard view support', () => {
    it('getTeacherData retrieves both teacher and assistant sessions', async () => {
      ...
      const submissionRepo = { find: jest.fn().mockResolvedValue([]), query: jest.fn().mockResolvedValue([{ count: 0 }]) };
